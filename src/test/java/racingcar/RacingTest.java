...
public class RacingTest {
    ...
    void singleWinnerTest() {
        ...
        // when: 우승자를 찾음
        List<Car> winnersList = computer.determineWinners();
    ...
    @DisplayName("자동차 목록이 비어있을 때 예외 발생 테스트")
    @Test
    void findWinnersEmptyCarsTest() {
        // given: 기본 생성자로 Computer 객체를 생성 (자동차 목록이 빈 상태)
        Computer emptyComputer = new Computer();

        // then: IllegalStateException 발생 여부 확인
        Exception exception = assertThrows(IllegalStateException.class, emptyComputer::determineWinners);
